...
// LicenseCmd: veil license install <file>, veil license show, veil license activate
func LicenseCmd() *cobra.Command {
	...
	c.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if s.IsPro() {
				// Register this machine in the activation log (one ping, no
				// usage data). Best-effort: never blocks Pro.
				fmt.Println()
				reportActivation(license.Activate(Version))
			}
			return nil
		},
	})
	c.AddCommand(&cobra.Command{
		Use:   "activate",
		Short: "Register this machine in your license's activation log (one ping)",
		Long: "Sends ONE logging ping so the activation record stays current. It is the\n" +
			"only network call Veil Pro makes, and it never gates anything: Pro works\n" +
			"whether or not this succeeds (offline, airgapped, or firewalled all keep\n" +
			"working).\n" +
			"\n" +
			"What is sent: your license token, the version, and a per-machine id that is\n" +
			"a ONE-WAY hash of your system machine-id (sha256, truncated). It is not your\n" +
			"hardware, MAC, hostname, or user, and it cannot be reversed. NO usage data is\n" +
			"ever sent. The free edition never pings.\n" +
			"\n" +
			"Use Veil on your own machines. If you replace one, run `veil license\n" +
			"deactivate` on the old machine to drop it from the record.",
		RunE: func(cmd *cobra.Command, args []string) error {
			s := license.LoadFromDefault()
			if !s.IsPro() {
				return fmt.Errorf("no valid Pro license installed (run: veil license install <file>)")
			}
			return reportActivation(license.Activate(Version))
		},
	})
	c.AddCommand(&cobra.Command{
		Use:   "deactivate",
		Short: "Remove this machine from your license's activation log",
		Long: "Drops this machine from your license's activation record (e.g. before you\n" +
			"sell or wipe it) so the record stays accurate. Logging-only, like activate.",
		RunE: func(cmd *cobra.Command, args []string) error {
			s := license.LoadFromDefault()
			if !s.IsPro() {
				return fmt.Errorf("no valid Pro license installed")
			}
			res, err := license.Deactivate(Version)
			if err != nil {
				fmt.Println("Could not reach the activation log:", err)
				return nil
			}
			if res.Machines > 0 {
				fmt.Printf("Removed. Your license is now logged on %d machine(s).\n", res.Machines)
			} else {
				fmt.Println("Removed this machine from your license's activation log.")
			}
			return nil
		},
	})
...
// reportActivation prints a buyer-facing summary of the logging ping. Activation
// never gates Pro, so any error is informational only.
func reportActivation(res license.ActivationResult, err error) error {
	if err != nil {
		fmt.Println("Could not reach the activation log:", err)
		fmt.Println("Veil Pro still works on this machine; this ping is only for the record.")
		return nil
	}
	switch {
	case res.Already:
		fmt.Println("This machine is already on your license's activation record.")
	case res.Machines > 0:
		fmt.Printf("Registered this machine. Your license is now logged on %d machine(s).\n", res.Machines)
	default:
		fmt.Println("Registered this machine on your license.")
	}
	return nil
}
